...
import io.core.libra.dtos.LoginModel;
...
class AuthControllerTest extends BaseTest {
    ...
    void testUserSignUp() throws Exception {
        ...
        userSignUp(userModel, "$.message", Messages.SUCCESS.getMessage(), status().isCreated());
    }
    ...
    void testUserSignUp_Fail_1() throws Exception {
        ...
        userSignUp(userModel, "$.message", Messages.VALIDATION_ERRORS.getMessage(), status().isBadRequest());
    }
    ...
    void testUserSignUp_Fail_2() throws Exception {
        ...
        mockMvc.perform(put(URL_BASE + "/signup")
                        .contentType(APPLICATION_JSON).content(asJsonString(userModel)))
    ...
    void testFindUserByEmail() throws Exception {
        LoginModel loginModel = new LoginModel("emmox55@gmail.com");
        userSearchByEmail(loginModel, "$.message", Messages.SUCCESS.getMessage(), status().isOk());
    }

    @Test
    @DisplayName("To Test a failed Login process using unregistered email")
    void testFindUserByEmail_Fail_1() throws Exception {
        LoginModel loginModel = new LoginModel("embox55@gmail.com");
        userSearchByEmail(loginModel, "$.message", Messages.NO_USER_RECORD_FOUND.getMessage(), status().isNotFound());
    }

    @Test
    @DisplayName("To Test a failed Login process using invalid inputs")
    void testFindUserByEmail_Fail_2() throws Exception {
        LoginModel loginModel = new LoginModel("embox55gmail.com");
        userSearchByEmail(loginModel, "$.message", Messages.VALIDATION_ERRORS.getMessage(), status().isBadRequest());
    }
    ...
    void testFindUserByEmail_Fail_3() throws Exception {
        LoginModel loginModel = new LoginModel("embox55gmail.com");
        mockMvc.perform(put(URL_BASE + "/search?email=" + loginModel.getEmail())
    ...
    private void userSignUp(UserModel userModel, String jsonPath, String jsonPathValue, ResultMatcher expectedStatus) throws Exception {
    ...
    private void userSearchByEmail(LoginModel loginModel, String jsonPath, String jsonPathValue, ResultMatcher expectedStatus) throws Exception {
        mockMvc.perform(get(URL_BASE + "/search?email=" + loginModel.getEmail())
                        .contentType(APPLICATION_JSON))
                .andDo(print())
                .andExpect(expectedStatus)
                .andExpect(jsonPath(jsonPath, Is.is(jsonPathValue)));
